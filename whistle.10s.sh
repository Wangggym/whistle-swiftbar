...
if [ "$1" = "toggle" ]; then
    ...
    if [ "$SHOW_NOTIFICATIONS" = "true" ]; then
        osascript -e "display notification \"Toggling $RULE_NAME...\" with title \"Whistle\"" &>/dev/null &
    fi
    ...
    # Trigger menu refresh after toggle
    open "swiftbar://refreshallplugins" &>/dev/null &
